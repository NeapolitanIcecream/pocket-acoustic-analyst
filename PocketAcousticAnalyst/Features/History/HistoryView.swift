               ...
               appModel.completedAnalyses.isEmpty
            {
                ...
                List {
                    if !appModel.completedComparisons.isEmpty {
                        Section("前后对比") {
                            ForEach(appModel.completedComparisons.sorted(by: { $0.measuredAt > $1.measuredAt })) { comparison in
                    ...
                    if !appModel.completedSpatialScans.isEmpty {
                        Section("位置扫描") {
                            ForEach(appModel.completedSpatialScans.sorted(by: { $0.measuredAt > $1.measuredAt })) { scan in
                    ...
                    if !standaloneAnalyses.isEmpty {
                        Section("声音检查") {
                            ForEach(standaloneAnalyses.sorted(by: { $0.measuredAt > $1.measuredAt })) { analysis in
    ...
    private var standaloneAnalyses: [AcousticAnalysis] {
        let nestedIDs = Set(
            appModel.completedSpatialScans.flatMap { scan in
                (scan.measurements + [scan.closureMeasurement]).map(\.analysis.id)
            }
        ).union(
        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                if let recommendation = scan.recommendation,
                   let point = scan.measurements.first(where: { $0.id == recommendation.recommendedMeasurementID })
                {
                    Text("\(point.label)：低约 \(recommendation.improvementDB.formatted(.number.precision(.fractionLength(1)))) dB")
                        .font(.headline)
                } else {
...
private struct ComparisonHistoryRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Image(systemName: comparisonIcon).foregroundStyle(AppTheme.accent)
            VStack(alignment: .leading, spacing: 4) {
