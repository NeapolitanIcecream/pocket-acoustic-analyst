...
6. `low-frequency-v4` 保留未达到持续性门槛的候选峰，并结合逐帧出现比例、频率离散、强弱离散和竞争峰，把有效录音描述为稳定、间歇、频率漂移、强弱变化、多个音调或能量分散。连续漂移要求逐帧最强峰形成步长受限的路径；相距较远、依次出现的频率不会被当作连续漂移。
...
- 有效的单次声音检查在结果出现时自动保存。归档还保存空间扫描和前后比较，不保存原始样本。候选峰和声音类型是 schema v1 的可选字段，旧记录缺少字段时按既有音调证据回退。P0 没有账号、云同步、遥测或后台录音。

## 已知边界

- 模拟器只验证流程和确定性输入，不验证麦克风频响、音频路由或 AR 漂移。
- 单次 20 秒分类描述的是该时间段内的频谱结构，不识别空调、风扇或电脑等声源。多个设备或负载变化会改变主要候选；需要同位置重复测量并逐一改变设备状态，才能积累来源证据。
